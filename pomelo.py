...
def createLeague(name):
    ...
    league = {'name': name, 'FOLDER': dir_path, 'JSON_DATA': file_path,
        'PLAYERS': {}, 'MATCHES': [], 'RANKING': [], 'LOGO': ''}
...
def updateLeague(league, player1, player2, result):
    ...
    if not found2:
        ...
        [newscore1, newscore2] = addScores(
            league, player1, player2, result)

        for id in range(len(league['PLAYERS'])):
            if league['PLAYERS'][str(id)]['name'] == player1:
                league['PLAYERS'][str(id)]['RANK'] = newscore1
                league['PLAYERS'][str(
                    id)]['MATCH'] = league['PLAYERS'][str(id)]['MATCH'] + 1
    ...
    league['MATCHES'].append(
        (player1, player2, result, '(' + dataora + ')'))
...
def updateRanking(league):
    ...
    for i in league['PLAYERS']:
        if(league['PLAYERS'][i]['RANK'] > 0):						# rank non negativi
            ...
            classification = sorted(classification, key=lambda player: (
...
def buildHtml(league_in):
    if(league_in == '_ALL_'):
        ...
        new_index_content = index_template.read().format(LEAGUE=league['name'], MATCH=match, RANKING=ranking, PLAYERS=PLAYERS, LOGO=logo_url)
        new_index.write(new_index_content)
        index_template.close()
        new_index.close()
